...
def build_features(df):
    ...
    if 'ball_no' not in df.columns:
        df['ball_no'] = (df['over'] * 6) + df['ball'] + 1
    ...
    if 'striker_out' not in df.columns:
        ...
        if 'player_dismissed' in df.columns:
            ...
            df['striker_out'] = df['bowler_wicket']
...
st.markdown(
    '<div class="ipl-title">🏏 IPL PREDICTOR</div>',
    unsafe_allow_html=True
)

st.markdown(
    '<div class="ipl-sub">Real IPL Data 2008–2024</div>',
    unsafe_allow_html=True
)
...
with st.spinner("Training ML models..."):
    ...
    reg_model, cls_model, reg_r2, reg_rmse, cls_acc = train_models(
        reg_df,
        cls_df
    )
...
with tab1:
    ...
    if st.button("Predict Final Score"):
        ...
        pred = int(reg_model.predict(X)[0])
...
with tab2:
    ...
    if st.button("Predict Winner"):
        ...
        crr = chase_score / max(overs2, 0.1)
        ...
        winner = defend_team if pred == 1 else chase_team
...
with tab3:
    ...
    st.write(f"R² Score: {round(reg_r2, 4)}")

    st.write(f"RMSE: {round(reg_rmse, 2)}")

    st.subheader("Classification Model")

    st.write(f"Accuracy: {round(cls_acc * 100, 2)}%")

    st.subheader("Dataset")

    st.write(f"Deliveries: {len(raw_df):,}")

    st.write(f"Regression Samples: {len(reg_df):,}")

    st.write(f"Classification Samples: {len(cls_df):,}")
